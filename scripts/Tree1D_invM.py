...
from scripts import beamSearchOptimal_invM as beamSearch
...
def plotBinaryTree(
	...
	def _rec(jet, parent, node_id):
		...
		if label:
			...
			# 	node_label = "pT:%0.1f\n m:%0.2f " % (np.linalg.norm(jet["content"][node_id][1:3]),
			# 										  np.sqrt(jet["content"][node_id][0] ** 2 - np.linalg.norm(jet["content"][node_id][1::]) ** 2)
			# 	                                                      # np.sqrt(jet["deltas"][node_id])
			# 														  )
			# else:
			# 	node_label = "pT:%0.1f" % (np.linalg.norm(jet["content"][node_id][1:3]))
			node_label = "pT:%0.1f\n m:%0.2f " % (np.linalg.norm(jet["content"][node_id][1:3]),
												  np.sqrt(jet["content"][node_id][0] ** 2 - np.linalg.norm(
													  jet["content"][node_id][1::]) ** 2)
												  # np.sqrt(jet["deltas"][node_id])
												  )
		else:
			node_label = ''""
	...
	if pySort:
		...
		ptList=[np.absolute(np.linalg.norm(jet["content"][node_id][1:3])) for node_id in outers]
	...
	if truthOrder:
		...
		if in_jet1["algorithm"] == "Truth":
			...
			if in_jet2["algorithm"] == "Beam Search":
				...
				in_jet1["LambdaRoot"] = in_jet2["LambdaRoot"]
				jetTop = beamSearch.recluster(in_jet1,
				                            delta_min=in_jet2["pt_cut"],
				                            lam=float(in_jet2["Lambda"]),
				                            beamSize = beamSize,
											visualize = True,
				                            N_best=N_best,
				                            save=False)[0]
